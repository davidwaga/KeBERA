...
function category_details(i){
    $.get(`${api_url}/categories/one.php?id=${i}`,(data,status)=>{
        console.log(data);
        $('#cname').text(data.categories.name);

        if(data.categories.length<0){
            $('#view-categories').html(`<div class='card'><div class='card-content text-center'>No Categories yet<br/><button class='btn btn-sm btn-success'>Add Category</button></div>`)
        }else{
            var pl = ''
            var k=1;
            for(var m of data.categories){
                pl+=`<tr>
                <td>${k}</td>
                <td>${m.name}</td>   
                <td>
